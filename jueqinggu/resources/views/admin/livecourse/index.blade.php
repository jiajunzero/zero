...
@section('title')直播课程列表@endsection
...
<div class="page-container">
	...
	{{--</div>--}}
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="course_add('添加直播课程','{{url('admin/livecourse/create')}}','1100','')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加直播课程</a></span> <span class="r">共有数据：<strong>{{$count}}</strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
        <tr>
            <th scope="col" colspan="12" style="text-align: center;font-size:20px">直播课程列表</th>
        </tr>
			<tr class="text-c">
...
$(function(){
    $('.table-sort').DataTable({
        ...
        'columnDefs':[{
            'targets':[0,2,3,4,-1],  //不参与排序的列
            'orderable':false
        }],
        'processing':true,   //显示数据在处理中的状态
        'serveSide':true,   //是否开启服务端
        'ajax':{
            'url':"{{url('admin/livecourse/ajax_list')}}",
            'type':'post',
        ...
        "columns": [
            ...
            {'data':'stream.stream_name',"defaultContent":''},
            {'data':'start_at',"defaultContent":' '},
            {'data':'sort',"defaultContent":" "},
            {'data':'created_at',"defaultContent":' '},
            {'data':'cao',"defaultContent":' '},
        ...
        "createdRow":function(row,data,dataIndex){
            ...
            $(row).addClass('text-c');
            $(row).find('td:eq(6)').html('开始: '+data.start_at+'<br>'+'结束: '+data.end_at);
            $(row).find('td:eq(5)').html('<span class="btn btn-success radius create_addr">'+data.stream.stream_name+'<span>');
...
//            $(row).find('td:eq(4)').html(data.disabled_at?data.disabled_at:'启用中...');
            ...
            $(document).on('click','.create_addr',function(){

                layer_show('直播流','/admin/live/addr/'+data.stream.id,800,500);
            });
