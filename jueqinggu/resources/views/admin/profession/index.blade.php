...
<div class="page-container">
    ...
    {{--</div>--}}
    <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="member_add('添加专业','{{url('admin/profession/create')}}','1100','')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加专业</a></span> <span class="r">共有数据：<strong>{{$count}}</strong> 条</span> </div>
    <div class="mt-20">
    <table class="table table-border table-bordered table-hover table-bg table-sort">
        <thead>
        <tr>
            <th scope="col" colspan="12" style="text-align: center;font-size:20px">专业列表</th>
        </tr>
            <tr class="text-c">
                ...
                <th width="90">点击量</th>
                <th width="100">专业时长(小时)</th>
                <th width="90">有效期(天)</th>
                <th width="70">价格(元)</th>
                <th width="40">排序</th>
...
$(function(){
    $('.table-sort').DataTable({
        ...
        'columnDefs':[{
            'targets':[0,-1,2,3,7],  //不参与排序的列
            'orderable':false
        }],
        'processing':true,   //显示数据在处理中的状态
        'serveSide':true,   //是否开启服务端
        'ajax':{
            'url':"{{url('admin/profession/ajax_list')}}",
            'type':'POST',
        ...
        "columns": [
            ...
            {'data':'pro_name',"defaultContent":" "},
            {'data':'profession_cate.cate_name',"defaultContent":''},
            {'data':'teacher_ids',"defaultContent":''},
            {'data':'click',"defaultContent":' '},
            {'data':'duration',"defaultContent":" "},
            {'data':'expired_at',"defaultContent":' '},
            {'data':'price',"defaultContent":' ','className':'td-status'},
            {'data':'sort',"defaultContent":' ','className':"td-manage"},
            {'data':'created_at',"defaultContent":' ','className':"td-manage"},
            {'data':'cao',"defaultContent":' ','className':"td-manage"},
        ],
...
//          //专业详情弹出层
            $(row).click(function(){
                layer.open({
                    ...
                    area: ['800px', '550px'], //宽高
                    content:'<h2>'+data.pro_name+'</h2>'+ data.detail
                });
            })

            //老师的昵称
            var teacher=[];
            $(data.teacher_ids).each(function(k,v){
               teacher.push(v.nickname)
            });

            teacher=teacher.join();
            $(row).find('td:eq(4)').html(teacher);
