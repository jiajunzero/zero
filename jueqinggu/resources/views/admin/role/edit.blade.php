	...
	<form action="{{url('admin/role/'.$roleInfo->id)}}" method="post" class="form form-horizontal" id="form-admin-role-add">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$roleInfo->role_name}}" id="roleName" name="role_name" datatype="*4-16" nullmsg="用户账户不能为空">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">备注：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$roleInfo->note}}" placeholder="" id="" name="note">
			</div>
		</div>
		<div class="row cl">
					...
					<dl class="permission-list">
						<dt>
							<label>
								...
								@foreach($roleInfo->role_auth_ids as $item) {{$item==$top->id?'checked':''}} @endforeach>
								{{$top->auth_name}}
							</label>
						</dt>
						<dd>
									...
									@foreach($sonAuth as $son)
										@if($son->auth_pid==$top->id)
											<label class="">
												...
												@foreach($roleInfo->role_auth_ids as $item) {{$item==$son->id?'checked':''}} @endforeach >
												{{$son->auth_name}}
											</label>
										@endif
									@endforeach
